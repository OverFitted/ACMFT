...
def preprocess_hr_signal(
    ...
    if bandpass_filter:
        signal = bandpass_filter_signal(signal, lowcut=lowcut, highcut=highcut, fs=target_sr, order=4)
...
def load_hr_signal(file_path: Union[str, Path]) -> Optional[np.ndarray]:
    """
...
def bandpass_filter_signal(signal: np.ndarray, lowcut: float, highcut: float, fs: float, order: int = 4) -> np.ndarray:
    """
...
def remove_signal_artifacts(signal: np.ndarray, threshold: float = 3.0) -> np.ndarray:
    """
...
def adjust_signal_length(signal: np.ndarray, target_length: int) -> np.ndarray:
    """
...
def extract_hr_features(signal: np.ndarray, fs: float = 256.0) -> Dict[str, np.ndarray]:
    """
...
def detect_r_peaks(signal: np.ndarray, fs: float = 256.0) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    # Find peaks
    peaks, _ = scipy.signal.find_peaks(filtered_signal, distance=min_distance, height=0.5 * np.std(filtered_signal))
...
def calculate_lf_hf_ratio(
    rr_intervals: np.ndarray, lf_band: Tuple[float, float] = (0.04, 0.15), hf_band: Tuple[float, float] = (0.15, 0.4)
) -> float:
    """
...
def augment_hr_signal(
    signal: np.ndarray, noise_level: float = 0.01, time_warp: bool = False, warp_factor: float = 0.1
) -> np.ndarray:
    """
